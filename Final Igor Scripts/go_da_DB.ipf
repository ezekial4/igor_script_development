...
Function go_DALPHA(endpnt,tspnts,killwav)
...
//	NewPath/O/Q DALPHParam "MainSSD:Users:unterbee:Desktop:DALPH_DB:"	
	...
	for(i=0;i<endpnt;i+=1)
		...
		if(wave1[i] > 0)
			...
			String cmdstr
			cmdstr =  "Load_NetCDF/D/Q/P=DALPHParam \"dalphaDB_"+shotnum+".nc\""
			Execute cmdstr
			
			Wave tstime = nc_TSTIMECO
			Wave tFS03 = nc_t_FS03
			Wave FS03 = nc_FS03
			Wave FS01 = nc_FS01
			Wave tFS01 = nc_t_FS01
			...
			if (Killwav == 1)
				KillWaves/Z nc_FS03, nc_t_FS03, nc_t_TSTIMECO
				KillWaves/Z nc_FS01, nc_t_FS01, truncdalp3, t_truncdalp3
				KillWaves/Z truncdalp1,t_truncdalp1, nc_TSTIMECO
